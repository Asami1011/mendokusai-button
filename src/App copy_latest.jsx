...
function App() {
  ...
  const [visibleIndexes, setVisibleIndexes] = useState([]); // フェードイン管理
  ...
  const getStepIcon = (step) => {
    ...
    if (/焼く|炒める|炒める|オーブン/.test(step)) return "🔥";
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (scrollTop + vh + 50 >= bodyHeight && visibleCount < recipeList.length) {
  ...
  // visibleCount が増えたらフェードイン用にインデックス追加
  useEffect(() => {
    const newIndexes = [];
